...
function buildInstructionCode (node, parentNode, index) {
  ...
  // Check if it's last atomic operation
  const isLastAtomicOperation = node.isLastAtomicOperation ? 'true' : 'false'

  // Check if it's looped
  const isLoop = node.loop ? 'true' : 'false'

  // Find nearest name group
  const head = JSON.stringify(node.head)

  // Build a code for instruction
  return `new Instruction(${nextSiblingIndex},${type},${parentIndex},${data},${children},${isLastAtomicOperation},${isLoop},${head})`
}
...
function buildInstructionInstance (node, parentNode, index) {
  ...
  // Check if it's last atomic operation
  const isLastAtomicOperation = !!node.isLastAtomicOperation

  // Check if it's looped
  const isLoop = !!node.loop

  // Find nearest named group
...
/**
 * Mark last atomic operations (either last in root node or in atomic group)
 *
 * @param {Node} rootNode
 */
function markLastAtomicOperations (rootNode) {
  rootNode.walk((node, parentNode, index) => {
    ...
    if (parentNode.type === 'Root' || parentNode.type === 'AtomicGroup' || (parentNode.type === 'Group' && parentNode.isLastAtomicOperation)) {
      if (index === parentNode.children.length - 1) {
        // It is last atomic operation
        node.isLastAtomicOperation = true
      }
    }
  })
}
...
/**
 * Mark loop operations (which don't have to go down in success, mostly in "Many possessive")
 *
 * @param {Node} rootNode
 */
function markLoopOperations (rootNode) {
  rootNode.walk((node, parentNode, index) => {
    // Ignore root node
    if (!parentNode) {
      return
    ...
    if (node.type !== 'AnyGreedy' && node.type !== 'ManyGreedy') {
      return
    }
    ...
    node.children[0].loop = true
  })
}

/**
 * Detect head of instruction (nearest named group)
 *
 * @param {Node} rootNode
 */
function detectInstructionsHead (rootNode) {
  rootNode.walk((node, parentNode) => {
    if (node.type === 'Group' && node.data.name !== void 0) {
      node.head = node.data.name
      return
    }

    node.head = parentNode && parentNode.head !== null ? parentNode.head : null
  })
}
...
function buildInstruction (rootNode, optimized = true) {
  ...
  // Mark last atomic operations for performance reasons
  // We are sure that we don't have to bailout from them after their finished
  markLastAtomicOperations(rootNode)

  // Mark loop operations
  markLoopOperations(rootNode)

  // Detect "head" of instructions
  detectInstructionsHead(rootNode)
