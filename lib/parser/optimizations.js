...
function removeRedundantNodes (node, parentNode, index) {
  ...
  if (node.type === 'Nothing') {
    ...
    if (parentNode.type === 'Group' || parentNode.type === 'Root') {
  ...
  if (node.children.length > 1 && parentNode.type !== 'Group' && parentNode.type !== 'Root') {
    return
  }
 ...
 * @param {int} index
 */
function simplifyManyLazyInstruction (node, parentNode, index) {
  // Ignore nodes which are not ManyLazy
  ...
  const optimizedNode = new Node('Group', [
    node.children[0],
    new Node('AnyLazy', node.children.map(x => x.clone()))
  ])

  // Replace ManyLazy instruction
  parentNode.children.splice(index, 1, optimizedNode)
}
...
exports.simplifyManyLazyInstruction = simplifyManyLazyInstruction
